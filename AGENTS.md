...
- The `changes` gate job is skipped when `github.repository == 'devopscoop/aws-eks-template'`, because the template repo has no real cluster; skipping it cascades to the `opentofu` job via `needs`. Forks and subtrees run both jobs normally.
- The `opentofu` job is a required status check on `main`, enforced by the branch ruleset in `.github/rulesets/require-opentofu-check.json` (applied once with `gh api` — see the README's "Protect the main branch" step). That is why the `pull_request` trigger has no `paths:` filter: a required check that never reports leaves the PR stuck on "Expected" forever, so the workflow runs on every PR and the `changes` job skips `opentofu` when nothing under `cluster/**` changed (a skipped job satisfies the check). Don't re-add a `paths:` filter to `pull_request`. For subtrees, quickstart.sh renames the check to `opentofu-<cluster_name>` so each cluster in a monorepo has its own required check.
